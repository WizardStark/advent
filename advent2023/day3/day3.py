...
def getnum(line, i):
    numend = i
    while numend < len(line) - 1 and line[numend + 1].isalnum():
        numend += 1
    return line[i : numend + 1], numend - i + 1


def check_adj(input, num, i, j, gearCoords, gearVals):
    h_start = max(0, j - 1)
    h_end = min(j + len(num) + 1, len(input[i]))
    v_start = max(0, i - 1)
    v_end = min(i + 2, len(input))
    isAdjacent = False
    for y in range(v_start, v_end):
        for x in range(h_start, h_end):
            ...
            if not val.isalnum() and val != ".":
                if val == "*":
                    if [y, x] in gearCoords:
                        idx = gearCoords.index([y, x])
                        gearVals[idx][0] += 1
                        gearVals[idx][1] *= int(num)
                    else:
                        gearCoords.append([y, x])
                        gearVals.append([1, int(num)])
                isAdjacent = True
    return [isAdjacent, gearCoords, gearVals]
...
def solution(input):
    ...
    for line in input:
        ...
        while j < w:
            if line[j].isalnum():
                num, skip = getnum(line, j)
                isAdjacent, gearCoords, gearVals = check_adj(
                    input, num, i, j, gearCoords, gearVals
                )
                total += isAdjacent * int(num)
                j += skip
            else:
                j += 1
        i += 1
