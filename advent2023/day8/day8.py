...
def parse(input):
    ...
    for i in range(1, len(input)):
        maps[input[i].split(" = ")[0]] = (
            input[i].split(" = ")[1].replace("(", "").replace(")", "").split(", ")
        )
...
def part1(input):
    instructions, maps = parse(input)
    prevKey = "AAA"
    i = 0
    count = 1
    instrCount = len(instructions)
    while True:
        prevKey = maps[prevKey][instructions[i] == "R"]
        if prevKey == "ZZZ" and i + 1 == instrCount:
            break
        count += (i + 1) // instrCount
        i = (i + 1) % instrCount
...
def part2(input):
    ...
    counts = [0] * len(prevKeys)

    instrCount = len(instructions)
    for knum, key in enumerate(prevKeys):
        i = 0
        while True:
            key = maps[key][instructions[i % instrCount] == "R"]
            i += 1
